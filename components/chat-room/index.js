...
import { View, Text, ScrollView, FlatList, Image, TouchableOpacity, StyleSheet, TextInput, KeyboardAvoidingView, Keyboard, TouchableWithoutFeedback, Platform, ShadowPropTypesIOS } from 'react-native';
...
const ChatRoom = props => {
  ...
  //var dynamicLoc = props.CurrentLocation;
  const submitMessage = () => {
    firestore()
      .collection('chats')
      .doc(props.RoomCode)
  ...
  const calculateDistance = (lat1, lon1) => {
    ...
    //let lat2 = dynamicLoc.coords.latitude;
    //let lon2 = dynamicLoc.coords.longitude;
            ...
            <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} style={styles.scrollViewStyles} contentContainerStyle={styles.scrollViewContainer}>
              ...
              {props.RoomUsers.map((user, index) => (user.uid !== props.Auth().currentUser.uid ? (
                  <View style={styles.userContainer} key={index}>
                    ...
                    <Text style={styles.userName}>{user.isTyping ? "is typing" : null}</Text>
                  </View>
                ) : (
                  null
                )
